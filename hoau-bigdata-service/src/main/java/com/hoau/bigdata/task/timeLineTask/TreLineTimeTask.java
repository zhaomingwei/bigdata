 ...
 * @version: $version
 */
@JobHandler(value = "TreLineTimeTask")
@Component
public class TreLineTimeTask extends IJobHandler {
    ...
    public ReturnT<String> execute(String param) throws Exception {
        try {
            ...
            logger.info("时间线路抽取异常--》" + e.getMessage());
            return FAIL;
        }
    }

    private void timeLineWriteHdfs() {
        logger.info("timeLineWriteHdfs--抽取线路时间数据");
...
//            HdfsFileUtils.writeToHdfs(hdfsConfig.hdfsPath, hdfsConfig.vehicleArrivalDirtmp, hdfsConfig.vehicleArrivalTimeLine + hdfsConfig.outNameType, stringContent, true, true, false);
            ...
            logger.info("时间线路--写入hdfs完成");
        } catch (Exception e) {
            logger.info("时间线路==写入HDFS异常++》" + e);
        }

    }

    private List<TimeLineEntity> findLineTime() {
        logger.info("findLineTime--抽取线路时间数据");
