 ...
 */
public class EasyInstallTask implements Callable<List<EasyInstallEntity>> {

    private static final Log log = LogFactory.getLog(EasyInstallTask.class);
    ...
    public EasyInstallTask(CountDownLatch countDownLatch, int sec) {
        this.countDownLatch = countDownLatch;
        this.sec = sec;
    }

    @Override
    public List<EasyInstallEntity> call() {
        log.info("============查询易安装明细并写入HDFS任务start=================");
        List<EasyInstallEntity> list = new ArrayList<>();
        try {
            list = trackDao.queryEasyInstall(sec);
            if (null == list || list.isEmpty()) {
                log.info("易安装结果为空");
            } else {
                log.info("易安装结果条数为：" + list.size());
            }
            hdfsService.writeToHdfs(hdfsConfig.baseClusterPath, hdfsConfig.easyInstall, hdfsConfig.easyInstallOutName,
