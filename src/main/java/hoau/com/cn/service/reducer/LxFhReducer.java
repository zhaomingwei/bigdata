...
/**
 * @Description: 效率监控看板-发货看板数据reducer处理
 * @Author: zhaowei
 * @Date: 2020/10/19
 * @Time: 16:48
 */
public class LxFhReducer extends Reducer<Text, Text, Text, Text> {
        ...
        try {
            ...
            if (!fhmxList.isEmpty()) {
                if (!timeLineList.isEmpty()) {
                    ...
                    for(FhMxAndDhMxOut fhMxAndDhMxOut : fhmxList){
                        context.write(key, new Text(fhMxAndDhMxOut.getStringContent()));
                    }
                }
            }
        } catch (Exception e) {
            System.out.println("LxFhReducer->exception:" + e);
        }
    }
